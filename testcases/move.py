...
# first page elements
move_house_xpath_elem = '//*[@id="user-journey-actions-form"]/div[2]/div/div/div[1]/label'
...
move_out_date_id_elem = 'Property[MoveInfo][MoveOutDate]'
...
no_final_meter_reading = '.bkgd-swap:nth-child(4) > .row:nth-child(1) .radio:nth-child(2) > label'
dont_send_meter_reading = '.row:nth-child(3) .table-row .radio:nth-child(2) > label'
power_switch_date = 'Property[MoveInfo][MoveInDate]'
no_vulnerable = "//label[contains(@for,'Property[MedicalInfo][HasVulnerablePerson]_NO')]"
...
addProperty = driver.find_element_by_xpath(move_house_xpath_elem)
addProperty.click()
...
#  enter address   #TODO change this to another address
addressPicker = driver.find_element_by_xpath(address_xpath_elem)
addressPicker.send_keys(test_data_address)
wait_for_elem(5)
addressPicker.send_keys(Keys.ARROW_DOWN)
...
#  moving out date
moving_out_date = driver.find_element_by_css_selector("#Property\[MoveInfo\]\[MoveOutDate\]")
moving_out_date.send_keys(test_data_move_out_date)
print('Moving Date -  entered!')

time.sleep(2)
# no meter reading
no_meter_reading = driver.find_element_by_css_selector(".bkgd-swap:nth-child(4) > .row:nth-child(1) .radio:nth-child(2) > label")
no_meter_reading.click()
print('No meter reading clicked!!')


# dont send meter reading
dontsend = driver.find_element_by_css_selector(dont_send_meter_reading)
dontsend.click()
print('Dont send meter reading - Selected!')

# move in date
# -------------------------------------------------------------------------------
time.sleep(5)
#TODO add move in date to test data - hard corded here for now
# movein = driver.find_element_by_xpath("//input[@name='Property[MoveInfo][MoveInDate]']")
movein = driver.find_element_by_id(power_switch_date)
movein.send_keys("12/12/2019")
print('Move in date - Entered!')

no_vulnerable_person = driver.find_element_by_xpath(no_vulnerable)
...
wait_for_elem(5)
assert driver.title == MOVE_HOUSE_SUCCESS_PAGE_TITLE
